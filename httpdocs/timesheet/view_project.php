...
<body>
	...
	<div class="container border">
		<div class="row p-3">
			...
			<div class="col-10">
								...
								<!-- <td align="center">
									<div class="form-inline">
										<?php if ($row['status'] != 'approved') : ?>
											<form action="<?= $base_url; ?>timesheet_model.php/updateTimesheet" id="form-approve" method="post">
												<input type="hidden" name="status" value="approved">
												<input type="hidden" name="date" value='["<?= $row['date']; ?>"]'>
												<input type="hidden" name="userid" value="<?= $_GET['userid']; ?>">
												<input type="hidden" name="timeid" value='<?= $row['id']; ?>'>
												<button type="submit" class="btn btn-sm btn-success rounded-0" id="btn-approve" title="Approve this item"><i class="fa fa-check"></i></button>
											</form>
											<?php $approved++; ?>
										<?php endif; ?>
										<?php if ($row['status'] != 'rejected') : ?>
											<form action="<?= $base_url; ?>timesheet_model.php/updateTimesheet" id="form-reject" method="post">
												<input type="hidden" name="status" value="rejected">
												<input type="hidden" name="date" value='["<?= $row['date']; ?>"]'>
												<input type="hidden" name="userid" value="<?= $_GET['userid']; ?>">
												<input type="hidden" name="timeid" value='<?= $row['id']; ?>'>
												<button type="submit" class="btn btn-sm btn-danger rounded-0" id="btn-reject" title="Reject this item"><i class="fa fa-times"></i></button>
											</form>
											<?php $rejected++; ?>
										<?php endif; ?>
									</div>
				...
				<div class="form-inline">
					<?php if ($approved > 0) : ?>
						<form action="<?= $base_url; ?>timesheet_model.php/updateTimesheet" id="form-approve-all" method="post">
							<input type="hidden" name="status" value="approved">
							<input type="hidden" name="date" value='<?= json_encode($dateData); ?>'>
							<input type="hidden" name="userid" value="<?= $_GET['userid']; ?>">
							<button type="submit" class="btn btn-success rounded-0 mr-3" id="btn-approve-all">Approved Timesheet</button>
						</form>
					<?php endif; ?>
					<?php if ($rejected > 0) : ?>
						<form action="<?= $base_url; ?>timesheet_model.php/updateTimesheet" id="form-reject-all" method="post">
							<input type="hidden" name="status" value="rejected">
							<input type="hidden" name="date" value='<?= json_encode($dateData); ?>'>
							<input type="hidden" name="userid" value="<?= $_GET['userid']; ?>">
							<button type="submit" class="btn btn-danger rounded-0" id="btn-reject-all">Reject Timesheet</button>
						</form>
					<?php endif; ?>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		$(document).on("click", "#btn-approve-all", function(e){
			e.preventDefault();
			if (confirm("Approve timesheet?")) {
				$("#form-approve-all").submit();
			}
		});
		$(document).on("click", "#btn-reject-all", function(e){
			e.preventDefault();
			if (confirm("Reject timesheet?")) {
				$("#form-reject-all").submit();
			}
		});
	</script>
</body>

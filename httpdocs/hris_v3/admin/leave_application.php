...
?>
    ...
    <div class="page">
      <div class="page-header">
          <h1 class="page-title"><i>Filling & Monitoring</i></h1>
      </div>
                        ...
                        <div class="panel nav-tabs-horizontal" data-plugin="tabs">
                          ...
                          <div class="panel-body">
                            <div class="tab-content">
                             ...
                             <div class="row">
                             <div class="col-md-2">
                                 <button type="button"  data-toggle="modal" data-target="#applyLeave" class="btn btn-block btn-primary ">File Leave</button>
                             </div>
                             <div class="col-md-2">
                               <button type="button"  data-toggle="modal" data-target="#leaveViewCount" class="btn btn-block btn-primary ">View Leave Count</button>
                           </div>
                           </div>
                                <br>
                                <table  class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                                  ...
                                  $query = "SELECT * FROM tbl_leavedetails INNER JOIN tbl_leave ON tbl_leavedetails.leaveID = tbl_leave.leaveID WHERE employeeCode = '$session' ";
                                  ...
                                  if($num>0){
                                    ...
                                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                                      ...
                                      if ($leaveStatus == "Cancelled" || $leaveStatus == "Declined"  || $dateNow > $leaveTo){
                                          $output = "<button type='button'  disabled data-toggle='modal' data-target='#cancelButton".$row['leaveDetailsID']."' class='btn btn-block btn-danger waves-effect waves-classic' data-id=" .  $row['employeeCode'] . "><i class='fa fa-times-circle'></i> Cancel</button>";
                                      }else{
                                          $output = "<button type='button'   data-toggle='modal' data-target='#cancelButton".$row['leaveDetailsID']."' class='btn btn-block btn-danger waves-effect waves-classic' data-id=" .  $row['employeeCode'] . "><i class='fa fa-times-circle'></i> Cancel</button>";
                                      }
        ...
        <div class="modal-dialog modal-lg">
            ...
            <table id='example3' class='table table-hover table-striped'>
                        <thead>
                          ...
                          $query = "SELECT * FROM tbl_leaveinfo INNER JOIN tbl_leave ON tbl_leaveinfo.leaveID = tbl_leave.leaveID  WHERE employeeCode = '$user' ";
                          $stmt = $con->prepare($query);
                          $stmt->execute();
                          $num = $stmt->rowCount();
                          if($num>0){
                  ...
                  <form enctype = "multipart/form-data" method="post" action='process_request_leave.php'  autocomplete="off">
                                ...
                                <div class="col-md-9">
                                  <select name="leaveType" required data-plugin="select2" class="form-control" style="width: 100%;">
                                    ...
                                    <?php
                                      ...
                                      $user = $_SESSION['user_id'];
                                      $zero = 0;
                                      $query = "SELECT * FROM tbl_leaveinfo INNER JOIN tbl_leave ON tbl_leaveinfo.leaveID = tbl_leave.leaveID WHERE employeeCode='$user' AND leaveRemaining > '$zero'";
                                      $stmt = $con->prepare($query);
                                      $stmt->execute();

                                      $query2 = "SELECT * FROM tbl_leaveinfo WHERE employeeCode='$user'";
                                      ...
                                      if($num>0){
                                       ...
                                       // if no records found
                                         ...
                                         echo "<option DISABLED readonly> No Leave Available</option>";
                                       }
                                      ?>
                                  </select>
                              ...
                              <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                  <?php
                                    $user = $_SESSION['user_id'];
                                    $query1 = "SELECT * FROM tbl_employees WHERE employeeCode = '$user'";
                                    ...
                                    if($num>0){
                                      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                                        ...
                                        <input type="hidden" name="search" value="<?php echo $row['employeeCode'];?>">

...
if (isset($_POST['approve'])){
  ...
  $q = "SELECT * FROM tbl_leaveinfo INNER JOIN tbl_leavedetails ON tbl_leaveinfo.employeeCode = tbl_leavedetails.employeeCode WHERE tbl_leavedetails.leaveDetailsID = '$leaveDetails' AND tbl_leaveinfo.leaveID = '$leaveType'";
  ...
  if ($leaveRemaining == 0){
    ...
    if($stmt->execute()){
      ...
      $queryUpdate5 = "UPDATE tbl_leaveinfo SET leaveRemaining=:leaveRemaining, leaveUsed=:leaveUsed WHERE employeeCode = '$employeeCode' AND leaveID = '$leaveID'";

      $stmt5 = $con->prepare($queryUpdate5);

      $q = "SELECT * FROM tbl_leaveinfo WHERE employeeCode = '$employeeCode' AND leaveID = $leaveID";
      $stmt9 = $con->prepare($q);
      $stmt9->execute();
